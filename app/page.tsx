import { LocationEntry } from "@/components/location-entry";

export default function HomePage() {
  return (
    <div className="grid-lines">
      <section className="mx-auto flex min-h-[calc(100vh-72px)] max-w-6xl flex-col justify-center gap-10 px-4 py-12 md:px-6">
        <div className="max-w-3xl">
          <p className="text-sm uppercase tracking-[0.28em] text-black/48">Toronto demo MVP</p>
          <h1 className="mt-4 font-display text-5xl font-semibold leading-[0.95] md:text-7xl">
            Find nearby support, then plan toward stability.
          </h1>
          <p className="mt-6 max-w-2xl text-lg text-black/65">
            Browse essential services without signing in. Use grounded chat for local questions.
            Create an account or log in only when you want a longer-term roadmap or saved places.
          </p>
        </div>
        <div className="grid gap-6 md:grid-cols-[1.05fr,0.95fr]">
          <LocationEntry />
          <section className="glass-panel rounded-4xl p-6 shadow-card">
            <p className="text-xs uppercase tracking-[0.22em] text-black/45">What this does</p>
            <div className="mt-4 grid gap-4">
              {[
                {
                  title: "Dashboard first",
                  body: "Food, shelters, clinics, showers, legal help, and other essentials stay visible without forcing chat."
                },
                {
                  title: "Grounded AI chat",
                  body: "Answers are limited to services already retrieved for the current location."
                },
                {
                  title: "Signed-in stability roadmap",
                  body: "Roadmap planning is focused on the next few weeks and months, not just tonight."
                }
              ].map((item) => (
                <div key={item.title} className="rounded-3xl border border-black/5 bg-white/70 p-5">
                  <h2 className="text-xl font-semibold">{item.title}</h2>
                  <p className="mt-2 text-black/65">{item.body}</p>
                </div>
              ))}
            </div>
          </section>
        </div>
      </section>
    </div>
  );
}
